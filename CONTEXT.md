...
- **只克隆入口规则（clone entry rule only）**：只复制 `subResolver=0` 的入口规则 N 份，子规则一字不改、UUID 不变；每份副本换新 UUID、加 `name` 后缀，并继续引用同一批子规则。区别于「复制规则」：后者是整批规则全换 UUID。

## Excel 侧

- **设备清单（device inventory）**：一份 xlsx，默认列 `设备IP` / `设备名称` / `设备厂商`。
- **原始日志（raw log）**：日志 xlsx 里存放日志正文的列，默认列名 `原始日志`。
- **键值对日志（kv log）**：管道分隔的 `key="value"` 日志形态（如 `node_ip="10.0.0.1"|||`），与 JSON 体相对；提取脚本双格式自动适配。
...
- **去重 Sheet（dedup sheet）**：按某列去重只保留每个值的首条所生成的 Sheet（`<列名>去重`）；与「拆分字段」的按值分组相对。
- **预检（inspect）**：跑提取/去重/拆分前的只读检查——日志格式识别、字段出现率、截断风险；不产生输出文件。
- **截断行（truncated row）**：源 Excel 被 32767 字符单元格上限切断的日志行；提取时尽力取到截断处，诊断标记 `TRUNCATED_TAIL`，值不完整且源头不可恢复。
- **抽查回对（verify sample）**：提取后随机抽 N 行，用独立正则从原文重新找值与输出比对；「原文有值、提取为空」判失败，用于捕获静默漏提。
